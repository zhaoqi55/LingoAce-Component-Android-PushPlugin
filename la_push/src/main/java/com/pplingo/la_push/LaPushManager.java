...
import com.pplingo.la_push.callback.CommPushCallBack;
import com.pplingo.la_push.callback.PushSeqCallBack;
import com.pplingo.la_push.manager.CommonPushManager;
...
public class LaPushManager implements CommonPushManager {
    ...
    private static LaPushManager mInstance;


    public static CommPushCallBack callBack;

    private LaPushManager() {


    }

    public static LaPushManager getInstance() {
        if (mInstance == null) {
            synchronized (LaPushManager.class) {
                if (mInstance == null) {
                    mInstance = new LaPushManager();
                }
            }
        }
        return mInstance;
     ...
     * @return
     */
    private LaPushMessageBean transformJPushMessage(JPushMessage message) {
        LaPushMessageBean pushResultBean = new LaPushMessageBean();
        pushResultBean.setAlias(message.getAlias());
        pushResultBean.setTags(message.getTags());
        pushResultBean.setErrorCode(message.getErrorCode());
        pushResultBean.setSequence(message.getSequence());
        pushResultBean.setCheckTag(message.getCheckTag());
        pushResultBean.setTagCheckStateResult(message.getTagCheckStateResult());
        pushResultBean.setTagCheckOperator(message.isTagCheckOperator());
        pushResultBean.setMobileNumber(message.getMobileNumber());
    ...
    // 主线程再返回数据
    public void onReceiveNotificationAuthorization(final HashMap<String, Object> map) {
        Log.d(TAG, "runMainThread:" + "map = " + map);
        Handler handler = new Handler(Looper.getMainLooper());
        handler.post(new Runnable() {
            @Override
            public void run() {
                callBack.onReceiveNotificationAuthorization(map);
            }
        });
    }
